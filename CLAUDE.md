...
| `/ws` push (`control_entrades_state.push_pending`) | AO → | `"inputs":{"1":true},"last_edges":[2],"edge_counts":{"2":3}` | Llegeix `control_entrades_state.inputs`, `control_entrades_state.last_edges`, `control_entrades_state.edge_counts` |
...
| Endpoint / WS | Direcció | Format | Efecte |
|--------------|----------|--------|--------|
| `/ws` push (`rellotge_state.push_pending`) | AO → | `"time":"14:32","day":"dimarts"` | Llegeix `rellotge_state.hour`, `.minute`, `.wday` |
