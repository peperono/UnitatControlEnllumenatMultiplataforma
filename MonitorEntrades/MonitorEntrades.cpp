#include "MonitorEntrades.h"
#include "../LogState.h"
#include <string>

MonitorEntrades::MonitorEntrades() noexcept
    : QP::QActive{Q_STATE_CAST(&MonitorEntrades::initial)}
{}

Q_STATE_DEF(MonitorEntrades, initial) {
    Q_UNUSED_PAR(e);
    subscribe(INPUT_CHANGED_SIG);
    subscribe(EDGE_DETECTED_SIG);
    return tran(&MonitorEntrades::running);
}

Q_STATE_DEF(MonitorEntrades, running) {
    QP::QState status;
    ...
    switch (e->sig) {
        ...
        case INPUT_CHANGED_SIG: {
            ...
            if (!detail.empty())
                log_append("MonitorEntrades", "<< INPUT_CHANGED_SIG", detail);
        ...
        case EDGE_DETECTED_SIG: {
            ...
            log_append("MonitorEntrades", "<< EDGE_DETECTED_SIG", detail);
            status = Q_HANDLED();
            break;
        }

        default: {
            status = super(&MonitorEntrades::top);
            break;
        }
    }
    return status;
